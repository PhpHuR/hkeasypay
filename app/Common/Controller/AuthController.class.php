...
class AuthController extends CommonController {
    ...
    protected function _initialize(){
        ...
        if(empty($_SESSION['aid'])){
            $this->redirect(__ROOT__."Admin/Login/login");
//			$this->error('还没有登录，正在跳转到登录页',U('Admin/Login/login'));
        ...
        if(count($menus_curr)>=4){
            ...
            $menus_name = M('auth_rule')->where("id in ($menus_curr[0],$menus_curr[1])")->field('title,name')->select();
...
//		获取数组最后的键值
        $count = count($menus_name)-1;
        header('content-type:text/html;charset=utf8');
        $this->assign('menus_name',$menus_name);
