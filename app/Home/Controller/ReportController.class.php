...
class ReportController extends HomebaseController
{
    ...
    public $end;   //结束时间
    public $uid;
    private $timestamp; //接收的时间字符串
    public function _initialize(){
        parent::_initialize();
        $userinfo = M('userinfo')->where(array('user_id' => $this->user['member_list_userinfoid']))->find();
        $this->uid = $userinfo['user_id'];
        $timestamp = I('reservation','');  //传递过来的时间
        if (empty($timestamp)) {
            $time = time()-2592000;
            $timestamp = date('Y-m-d',$time) . ' - ' . date('Y-m-d'); //初始化时间段
        }
        $this->timestamp = $timestamp;
        $arr = explode(" - ", $timestamp);//转换成数组
        $begin_time = $arr[0];
        $end_time =$arr[1];
...
//        $this->assign('result', json_encode($result));
    ...
    public function daily()
    {
        ...
        $sql .= " where uid = $this->uid and end_time>$this->begin and end_time<$this->end AND status=1 group by gap";
        $res = M()->query($sql);//订单数,交易额
...
//        for ($i = $this->begin; $i <= $this->end; $i = $i + 24 * 3600) {
        ...
        $last_all_e[] = array(
            'day' => '總共：',
            'iordermoney' => $res_a['aiordermoney'],
            'ifree' => $res_a['aifree'],
            'otransfermoney' => $res_o_a['aotransfermoney'],
            'ofree' => $res_o_a['aofree'],
        );

        $allArray = array_merge($list,$last_all_e);
        ...
        if($action == 'export'){
            ...
            $this->goods_export($allArray,'daily');
        }
        $this->assign('begin_time', date('Y-m-d',$this->begin));
        $this->assign('end_time', date('Y-m-d',$this->end));
//        dump($this->timestamp);die;
        $this->assign('sldate', $this->timestamp);
        $this->assign('list', $list);
...
//        for ($i = $begin; $i <= $end; $i = $i + 24 * 3600 * 31) {  修改为降序
        for ($i = $end; $i >= $begin; $i = $i - 24 * 3600 * 31) {
        ...
        if($action == 'export'){
            ...
            $this->goods_export($allArray,'monthly');
        }
        $this->assign('sldate', $timegap);
        $this->assign('list', $list);
        $this->assign('list_all', $list_all);
