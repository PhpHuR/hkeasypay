...
class PayoutController extends AuthController
{
    function index()
    {
...
//                    $sumcount = $userinfo_list['sumcount'] - $transfermoney - $allfree;  //总金额
                    ...
                    $payout_data = array(
                        ...
                        'transfermoney_hkd' => isset($transfermoney_hkd) ? $transfermoney_hkd : 0,
                        'status' => 1,
                        'free' => $free,
                        'begin_time' => time(),
                        'remark' => I('remark'),
...
//            $sldate = I('reservation', '');//获取格式 2015-11-12 - 2015-11-18
    ...
    public function turnFoOrderPage()
    {
        ...
        if (IS_AJAX) {
            ...
            $map['payout_orderid|api_china_name|title'] = array('like', "%" . $key . "%");;
    ...
    public function turnFoAuditOrderPage()
    {
        ...
        if (IS_POST and IS_AJAX) {
            ...
            $Page = getpage($count, 10);// 实例化分页类 传入总记录数和每页显示的记录数
            $show = $Page->show();// 分页显示输出
            $audit_list = M('payout_list')->where($map)->limit($Page->firstRow . ',' . $Page->listRows)->order('payout_id desc')->select();
    ...
    public function turnFoOrderIng()
    {
        ...
        if (IS_POST and IS_AJAX) {
            ...
            $sl_data['bankname'] = getBankName($payout_audit['bankname']);
            $sl_data['transfermoney'] = $payout_audit['transfermoney'];
            $sl_data['transfermoney_hkd'] = $payout_audit['transfermoney_hkd'];
    ...
    public function runFoOrderIng()
    {
        if (!IS_AJAX) {
            ...
            if ($status == 4) {
                ...
                if (in_array($user_id, C('TMPL_HKD'))) {
                    $transfermoney_hkd = M("payout")->where(array("user_id" => I('uid'), 'payout_orderid' => I('payout_orderid')))->getField('transfermoney_hkd');
                    $sumcount = $balance_list['sumcount'] - $transfermoney_hkd - $free;  //总金额
                    $unsettlement = $balance_list['unsettlement'] - $transfermoney_hkd - $free; //未结算金额
                } else {
                ...
                if ($u_userinfo) {
                    ...
                    if (in_array(I('uid'), C('TMPL_HKD'))) {
                        $order_log_data_transfermoney = array(
                            ...
                            'currency_id' => $currency_id,
                            'income' => '0.00',
                            'outlay' => M("payout")->where(array('uid' => I('uid'), 'payout_orderid' => I('payout_orderid')))->getField('transfermoney_hkd'),
                            'outlay_hkd' => $transfermoney,
                        ...
                        $order_log_data_transfermoney = array(
                            ...
                            'outlay_hkd' => isset($outlay_hkd) ? $outlay_hkd : 0,
                            'balance' => $sumcount + $free,
                            'begin_time' => I('begin_time'),
                            'end_time' => $end_time,
                            'remark' => I('remark'),
                    ...
                    if ($rst !== false) {
                        //                            发送出金短信信息
                        $sms_data = M('sms_list')->where(array('u_id' => I('uid')))->find();
                        //验证是否启用和有可用条数
                        if ($sms_data['sms_state'] == 1 && $sms_data['sms_count'] > 0) {
                            $send['content'] = $sms_data['sms_payout_text'];
...
//                                扣发送数
                                $count['sms_count'] = $sms_data['sms_count'] - 1;
                                $res = M('sms_list')->where(array('u_id' => I('uid')))->save($count);
                                if (!$res) {
                                    $this->error('短信扣费失败', U('turnFoOrderIng'), 0);
                                }
                            }
                        }
