...
class IndexController extends AuthController {
	public function index(){
        ...
        //今日总入金手续费
        $payin_order_free = M('payin')->where(array('status' => 1, 'begin_time' => array('gt', strtotime(date('Ymd')))))->sum('free');
        $this->assign('payin_order_free',$payin_order_free);
        //出金总金额
        $payout_order_sum = M('payout')->where(array('status' => 4, 'begin_time' => array('gt', strtotime(date('Ymd')))))->sum('transfermoney');
        $this->assign('payout_order_sum',$payout_order_sum);
        //出金总手续费
        $payout_order_free = M('payout')->where(array('status' => 4, 'begin_time' => array('gt', strtotime(date('Ymd')))))->sum('free');
        $this->assign('payout_order_free',$payout_order_free);

        //总利润
        $pay_order_all = $payin_order_free + $payout_order_free;
        $this->assign('pay_order_all',$pay_order_all);
