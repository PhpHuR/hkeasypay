...
//		$this->assign('member_group',$member_group);
	...
	public function member_runadd(){
		if (!IS_AJAX){
			...
			$access->add($accdata);
			if ($access !== false) {
				$this->success('操作员添加成功', U('member_list'), 1);
			} else {
				$this->error('操作员添加失败', U('member_list'), 0);
			}
		}
	}
...
//		$member_group=M('member_group')->order('member_group_order')->select();
		...
		$this->assign('province',$province);
		//原始权限板块
		$map['uid']  =array('gt',10000);
		$map['group_id']  = $member_list_edit['member_list_groupid'];
		$map['_logic']  = 'AND';
		$auth_group_access=M('auth_group_access')->where($map)->getField('group_id');
