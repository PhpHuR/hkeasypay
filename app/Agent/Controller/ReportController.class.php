...
class ReportController extends AgentbaseController
{
...
//        $this->assign('begin_time', date('Y-m-d', $this->begin));
//        $this->assign('end_time', date('Y-m-d', $this->end));
...
//        die();
    ...
    function commission_report()
    {
        ...
        $userIdArray = implode(",", $this->userIdArray);
        //入金统计报表
        ...
        $sqll .= " where end_time>$this->begin and end_time<$this->end AND status=1 ";
        if ($userIdArray) {
            $sqll .= "and uid in ($userIdArray)";
        } else {
            $sqll .= "and uid = 0";
        }
        ...
        if ($userIdArray) {
            $userInfoWhere['user_id'] = array('in', $userIdArray);
        } else {
            $userInfoWhere['user_id'] = '0';
        }

        $user_list = M('userinfo')->where($userInfoWhere)->select();
        foreach ($user_list as $key => $value) {
            ...
            $agentRate = getUserAgentRateNoProductId($value['user_id'], session('hid'));
...
//        die();
    ...
    public function daily()
    {
        $agentid = session('hid');
        //入金统计报表
        $sql = "SELECT COUNT(*) as inum,sum(order_money) as iordermoney,sum(agentfree) as ifree, FROM_UNIXTIME(update_time,'%Y-%m-%d') as gap from  __PREFIX__agentpayin ";
        $sql .= " where update_time>$this->begin and update_time<$this->end ";
        $sql .= "and agent_pid =$agentid ";
        $sql .= " group by gap";
        $res = M()->query($sql);//订单数,交易额

        $map['update_time'] = array(array('gt', $this->begin), array('lt', $this->end), 'and');
        $map['agent_pid'] = session('hid');
        $res_a = M('agentpayin')->where($map)->field("sum(`order_money`) as `aiordermoney`,sum(`agentfree`) as `aifree`")->find();
...
//            $lastweek = date('Y-m-d',strtotime("-1 week"));//7天前
        ...
        $agentid = session('hid');
        //入金统计报表
        $sql = "SELECT COUNT(*) as inum,sum(order_money) as iordermoney,sum(agentfree) as ifree, FROM_UNIXTIME(update_time,'%Y-%m') as gap from  __PREFIX__agentpayin ";
        $sql .= " where update_time>$this->begin and update_time<$this->end ";
        $sql .= "and agent_pid =$agentid ";
        $sql .= " group by gap";
        $res = M()->query($sql);//订单数,交易额
        $map['update_time'] = array(array('gt', $begin), array('lt', $end), 'and');
        $map['agent_pid'] = $agentid;
        $res_a = M('agentpayin')->where($map)->field("sum(`order_money`) as `aiordermoney`,sum(`agentfree`) as `aifree`")->find();
        $inum = "";
        $ifree = "";
        $iordermoney = "";
