...
/*入金订单通用Ajax*/
$(function () {
    $(".payin-state-btn").click(function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {x: val}, function (data) {
            if (data.status) {
                if (data.info == '未通知') {
                    ...
                    var b = '<button class="btn btn-minier btn-yellow">已通知</button>';
                    $('#zt' + val).html(b);
                    return false;
                }
            } else {
...
/*入金订单通用Ajax*/
$(function () {
    $(".payin-repair-btn").click(function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {x: val}, function (data) {
            if (data.status) {
                if (data.info == '已補單') {
                    ...
                    var b = '<button class="btn btn-minier btn-yellow">已補單</button>';
                    $('#zx' + val).html(b);
                    return false;
                }
            } else {
...
/*商戶交易詳細詳情 */
$(function () {
    $(".order_log-btn").click(function () {
        var $url = this.href,
            val = $(this).data('id');
        $.post($url, {id: val}, function (data) {
            // console.log(data);
            $("#myModaledit").show(300);
            $("#editorder_log_id").val(data.order_log_id);
            $("#edituid").val(data.uid);
            $("#editbatchnum").val(data.batchnum);
            $("#editreaccname").val(data.reaccname);
            $("#editreaccno").val(data.reaccno);
            $("#editbankname").val(data.bankname);
            $("#edittransfermoney").val(data.transfermoney);
            $("#edittransfermoney_hkd").val(data.transfermoney_hkd);
            $("#editfree").val(data.free);
...
$(function () {
    $(".payout-btn").click(function () {
        ...
        $.post($url, {id: val}, function (data) {
            if (data.status == 0) {
                $("#myModaledit").show(300);
                $("#editpayout_id").val(data.payout_id);
                $("#editcurrency_id").val(data.currency_id);
                $("#edituid").val(data.uid);
                $("#editbatchnum").val(data.batchnum);
                $("#editreaccname").val(data.reaccname);
                $("#editreaccno").val(data.reaccno);
    ...
    $(".payoutAudit").click(function () {
        ...
        $.post($url, {id: val}, function (data) {
            if (data.ok >= 1) {
                $("#myModaledit").show(300);
                $("#editpayout_id").val(data.payout_id);
                $("#editpayout_mid").val(data.payout_mid);
                $("#editattribute").val(data.attribute);
                $("#editcurrency_id").val(data.currency_id);
                $("#edituid").val(data.uid);
                $("#editbatchnum").val(data.batchnum);
                $("#editpayout_orderid").val(data.payout_orderid);
                $("#editreaccname").val(data.reaccname);
                $("#editreaccno").val(data.reaccno);
                $("#editbankname").val(data.bankname);
                $("#edittransfermoney").val(data.transfermoney);
                $("#editbegin_time").val(data.begin_time);
                $("#editaudit_time").val(data.audit_time);
                $("#editresult_time").val(data.result_time);
                $("#editremark").val(data.remark);
                $("#editresult_remark").val(data.result_remark);
                $("#editfree").val(data.free);
                $("#editstatus").val(data.status);
            } else {
                layer.alert(data.info, {icon: 5});
...
function getOrderNumber(number, maxNumber)
{
    if (number == 0) {
        ...
        var num1 = Math.ceil(number/maxNumber);//获取数量
        var arr = new Array();
 ...
 * @returns {Boolean}
 */
function checkPaymentFile(filePath) {
    var fileType = filePath.substring(filePath.lastIndexOf(".") + 1, filePath.length).toLowerCase();
    if (fileType != "xls" && fileType != "xlsx" && fileType != "txt" && fileType != "text") {
        return false;
    }
    return true;
}
