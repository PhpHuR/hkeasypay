 ...
 */
	...
	//产品分类用
	static public function menu_k($cate , $lefthtml = '─' , $pid=0 , $lvl=0, $leftpin=0 ){
		$arr=array();
		foreach ($cate as $v){
			if($v['parentid']==$pid){
				$v['lvl']=$lvl + 1;
				...
				$v['lefthtml']='├'.str_repeat($lefthtml,$lvl);
				$arr[]=$v;
				$arr= array_merge($arr,self::menu_k($cate,$lefthtml,$v['category_id'],$lvl+1 , $leftpin+20));
			}
		}
		return $arr;
	}
	...
	//代理层级
	static public function menu_j($cate , $lefthtml = '─' , $pid=0 , $lvl=0, $leftpin=0 ){
		$arr=array();
		foreach ($cate as $v){
			if($v['agent_list_pid']==$pid){
				$v['lvl']=$lvl + 1;
				$v['leftpin']=$leftpin + 0;//左边距
				$v['lefthtml']='├'.str_repeat($lefthtml,$lvl);
				$arr[]=$v;
				$arr= array_merge($arr,self::menu_j($cate,$lefthtml,$v['agent_list_id'],$lvl+1 , $leftpin+20));
			}
		}
		return $arr;
	}
