...
class ysepay_service
{
    ...
    {
        $this->param = array();
        $this->param['businessgatecerpath'] = 'http://'.$_SERVER['HTTP_HOST'].'/cert/4/businessgate.cer';
        $this->param['host'] = "pay.ysepay.com";//生产环境需更换为：pay.ysepay.com  测试：113.106.160.201:889
        $this->param['host_s'] = "https://openapi.ysepay.com/gateway.do";//生产环境需更换为：pay.ysepay.com  测试：113.106.160.201:889
    ...
    /**
     * 加急付款到银行
     * @功能：构造S1032报文，返回包含请求报文的url的数组
     * @param array $input
     * @return success
     * @return msg
     * @return url
     */
    function S1032_ysepay($input)
    {
        $MsgCode = "S1032";
    ...
    function S3001_ysepay($input)
    {
        ...
        $param = array(
            ...
            "businessgatecerpath" => $input['businessgatecerpath'],
            "pfxpath" => $input['pfxpath'],
            "pfxpassword" => $input['pfxpassword'],
            "noticepg_url" => $input['noticepg_url'],
            "noticebg_url" =>  $input['noticebg_url'],
        );
        ...
        $private_cert = $param['pfxpath'];
        $pkey = $param['pfxpassword'];

        //加密
        $sign_encrypt = self::sign_encrypt(array('data' => $xml),$private_cert,$pkey);
    ...
    function unsign_crypt($input)
    {
        ...
        $success = openssl_verify($msg, $check, openssl_get_publickey($certificateCApemContent), OPENSSL_ALGO_MD5);
        //openssl_public_decrypt (base64_decode($check),$finaltext,openssl_get_publickey($certificateCApemContent));
     ...
     * @return msg
     */
    function sign_encrypt($input,$private_cert,$pkey)
    {
        ...
        $pkcs12 = file_get_contents($private_cert); //私钥

        if (openssl_pkcs12_read($pkcs12, $certs, $pkey)) {
     ...
     *
     */
    function buildxml_S1032($param, $xml = null)
    {
        $xml =
        ...
        $xml =
			...
			<yspay>
				...
				<body>
					...
					<Payee>
						<UserCode>' . $param['usercode'] . '</UserCode>
						<Name>' . $param['merchantname'] . '</Name>
						<Amount>' . $param['amount_units_fen'] . '</Amount>
					</Payee>
					<Notice>
						<PgUrl>' . $param['noticepg_url'] . '</PgUrl>
						<BgUrl>' . $param['noticebg_url'] . '</BgUrl>
					</Notice>
				</body>
			</yspay>
		';
